# highlights/tests/test_highlight_api.py
...
class HighlightPermissionTests(APITestCase):
    ...
    def test_admin_can_create_highlight(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {get_token(self.admin)}")
        data = {"course": self.course.id, "label": "Admin Highlight", "type": "badge"}
        response = self.client.post(self.url, data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Highlight.objects.count(), 1)

    def test_instructor_can_create_highlight(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {get_token(self.instructor)}")
        data = {"course": self.course.id, "label": "Instructor Highlight", "type": "text"}
        response = self.client.post(self.url, data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_student_cannot_create_highlight(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {get_token(self.student)}")
        data = {"course": self.course.id, "label": "Forbidden Highlight", "type": "badge"}
        response = self.client.post(self.url, data)
