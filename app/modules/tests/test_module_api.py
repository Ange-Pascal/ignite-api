# modules/tests/test_module_api.py
...
class ModulePermissionTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.course = Course.objects.create(
            user=self.instructor,   # le créateur du cours
    ...
    def test_admin_can_create_module(self):
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {get_token(self.admin)}"
        )
    ...
    def test_instructor_can_create_module(self):
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {get_token(self.instructor)}"
        )

        data = {
            "course": self.course.id,
            "title": "Instructor Module",
            "description": "Module created by instructor",
            "position": 1
        }
    ...
    def test_student_cannot_create_module(self):
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {get_token(self.student)}"
        )
